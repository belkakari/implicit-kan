...
class LearnableActivation(nn.Module):
    def __init__(
        ...
        grid = (torch.arange(-spline_order, grid_size + spline_order + 1) * h + grid_range[0]).contiguous()
        self.register_buffer("grid", grid)

        self.spline_weight = torch.nn.Parameter(torch.Tensor(1, 1, grid_size + spline_order))
        if enable_standalone_scale_spline:
            self.spline_scaler = torch.nn.Parameter(torch.Tensor(1, 1))
    ...
    def reset_parameters(self):
        ...
        with torch.no_grad():
            noise = (torch.rand(self.grid_size + 1, 1, 1) - 1 / 2) * self.scale_noise / self.grid_size
